...
public class GridsModel : PageModel
{
    ...
    private readonly ILogger<GridsModel> _logger;
    private readonly Contexto _contexto;

    public GridsModel(ILogger<GridsModel> logger, Contexto contexto)
    {
        _contexto = contexto;
        _logger = logger;

        CadastroMkd = _contexto.Cadastro.FirstOrDefault(c => c.Id == 4);
        if (CadastroMkd == null)
        {
    ...
    public void OnGet()
    {
        var cadastro = _contexto.Cadastro.FirstOrDefault(c => c.Id == 4);
        if (cadastro != null)
        {
    ...
    {
        var registroExistente = await _contexto.Cadastro.FirstOrDefaultAsync(c => c.Id == 4);
